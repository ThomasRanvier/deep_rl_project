...
class Agent():
    ...
    def run_episode(self, epsilon, update_target_net = False, save_models = False):
        ...
        if save_models:
            torch.save(self._policy_net, MODELS_DIR + 'policy_net_' + FILE_SUFFIX + '_backtb.pt')
            torch.save(self._target_net, MODELS_DIR + 'target_net_' + FILE_SUFFIX + '_backtb.pt')
        return total_reward